...
export const Layout: React.FC<LayoutProps> = ({ children }) => {
  ...
  if (activeRole === 'admin') {
    menuItems.push(
      ...
      { label: 'Admins Master', path: '/users', icon: UserCheck },
                          ...
                          {user.roles.map((role: string) => {
                            ...
                            return (
                              <button
                                ...
                                className={`w-full text-left px-3.5 py-2 text-xs font-semibold flex items-center justify-between transition-colors ${isCurrent
                  ...
                  {menuItems.map((item) => {
                    ...
                    return (
                      <button
                        ...
                        className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-semibold transition-all ${isActive ? 'bg-primary-600 text-white' : 'hover:bg-slate-800'
                          }`}
                      >
                        <Icon className="w-5 h-5" />
                        <span>{item.label}</span>
