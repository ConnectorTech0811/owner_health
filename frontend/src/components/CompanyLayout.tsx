...
export const CompanyLayout: React.FC<CompanyLayoutProps> = ({ children }) => {
  ...
  const activeRole = localStorage.getItem('activeRole') || user.tipo_usuario || '';
  const isSecretaria = activeRole === 'secretaria' || activeRole === 'secretario';
    ...
    if (isCollapsed) {
      return (
        <button
          key={item.path}
          onClick={() => {
            if (isSecretaria && (item.path === '/company/prescriptions' || item.path === '/company/anamnesis-config')) {
              alert('Acesso Restrito: Perfis Administrativos e Secretárias não possuem acesso a esta tela.');
              return;
            }
            navigate(item.path);
            onClick?.();
